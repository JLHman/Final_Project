    ...
    if returnTable:
        if includeLinks:
            return render_template('TableList.html', values=tags, colnames=['tagName', 'dataType'], title='Tag List',
                                   url1Title='Discover Devices: ',
                                   url1Link=url_for('get_devices', ipAddress=ipAddress, slot=slot, _external=True))
        else:
            return render_template('TableList.html', values=tags, colnames=['tagName', 'dataType'], title='Tag List')
    else:
        if includeLinks:
            return render_template('TagList.html', tags=tags, url1Title='Discover Devices: ',
                                   url1Link=url_for('get_devices', ipAddress=ipAddress, slot=slot, _external=True))
        else:
...
def get_tag(tag, ipAddress, slot):
    ...
    if tag.startswith('[') and tag.endswith(']'):  # array of mixed tags
        ...
        if len(regularTags) > 0 or len(arrayTags) > 0:
            if returnTable:
                if includeLinks:
                    return render_template('TableList.html', values=results, colnames=['tagName', 'value', 'status'],
                                           title='Tags Values', url1Title='List Tags: ',
                                           url1Link=url_for('get_all_tags', ipAddress=ipAddress, slot=slot,
                                                            _external=True), url2Title='Discover Devices: ',
                                           url2Link=url_for('get_devices', ipAddress=ipAddress, slot=slot,
                                                            _external=True))
                else:
                    return render_template('TableList.html', values=results, colnames=['tagName', 'value', 'status'],
                                           title='Tags Values')
                ...
                if includeLinks:
                    return render_template('TagValues.html', results=results, url1Title='List Tags: ',
                                           url1Link=url_for('get_all_tags', ipAddress=ipAddress, slot=slot,
                                                            _external=True), url2Title='Discover Devices: ',
                                           url2Link=url_for('get_devices', ipAddress=ipAddress, slot=slot,
                                                            _external=True))
            ...
            if includeLinks:
                return render_template('TagValues.html', results='', url1Title='List Tags: ',
                                       url1Link=url_for('get_all_tags', ipAddress=ipAddress, slot=slot, _external=True),
                                       url2Title='Discover Devices: ',
                                       url2Link=url_for('get_devices', ipAddress=ipAddress, slot=slot, _external=True))
            else:
    ...
    if returnTable:
        if includeLinks:
            return render_template('TableList.html', values=devices, colnames=['productName', 'revision'],
                                   title='Device List', url1Title='List Tags: ',
                                   url1Link=url_for('get_all_tags', ipAddress=ipAddress, slot=slot, _external=True))
        else:
            return render_template('TableList.html', values=devices, colnames=['productName', 'revision'],
                                   title='Device List')
        ...
        if includeLinks:
            return render_template('DeviceList.html', devices=devices, url1Title='List Tags: ',
                                   url1Link=url_for('get_all_tags', ipAddress=ipAddress, slot=slot, _external=True))
        else:
